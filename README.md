...
- Zero setup beyond running Codex or Claude Code: it reads `~/.codex/sessions` and `~/.claude/projects` to infer status from logs.
...
Start or continue Codex or Claude Code sessions and the menu bar item updates automatically.

## How It Works

1. Finds live Codex and Claude Code processes and their session logs in `~/.codex/sessions` and `~/.claude/projects`.
...
- Node.js + npm (only if you edit `Sources/AgentBuffer/Resources/metrics-web/app.ts`)
- Codex CLI (sessions written to `~/.codex/sessions`) and/or Claude Code CLI (sessions written to `~/.claude/projects`)
...
If you edit `Sources/AgentBuffer/Resources/metrics-web/app.ts`, run `npm install` once so the build scripts can regenerate `app.js`.
...
**No agents detected?**  
AgentBuffer only counts **live Codex or Claude Code processes** with active session logs. Make sure Codex is running and writing to `~/.codex/sessions`, or Claude Code is running and writing to `~/.claude/projects`.
