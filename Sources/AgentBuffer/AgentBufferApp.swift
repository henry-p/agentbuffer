...
final class AgentBufferApp: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    private func showNotificationPromptWindow() {
        ...
        let label = NSTextField(labelWithString: "Requesting notification permission…")
        label.alignment = .center
        label.frame = NSRect(x: 16, y: (size.height - 20) / 2.0, width: size.width - 32, height: 20)
        ...
        refreshNotificationSettings { [weak self] status in
            ...
            if status == .notDetermined {
                self.requestAuthorizationWithPrompt { [weak self] granted in
                    ...
                    let idlePercent = self.idlePercent(for: snapshot)
    ...
    private func update(snapshot: StatusSnapshot) {
        ...
        let summary = "\(displaySnapshot.runningCount) out of \(displaySnapshot.totalCount) agents are currently running."
        let suggestionText = displaySnapshot.runningCount < displaySnapshot.totalCount
            ? "Keep all of your agents running to maximize efficiency."
            : nil
        popoverCoordinator?.updateMain(
            summary: summary,
            suggestion: suggestionText,
            hideSuggestion: suggestionText == nil,
    ...
    private func makeSimulatedSnapshot(showAllHistory: Bool) -> (snapshot: StatusSnapshot, recentAgents: [AgentListItem], canLoadMore: Bool) {
        ...
        let historyAll: [AgentListItem] = [
            ...
            AgentListItem(id: "sim-recent-10", type: .codex, title: "Update memory bank with UI changes")
        ]
    ...
    private func currentPidSignature() -> String {
        let pids = reader.currentCodexPids().sorted()
        return pids.map(String.init).joined(separator: ",")
    }

    private func setupFileWatcher() {
        let watcher = FileWatcher(url: reader.codexSessionsURL) { [weak self] in
            DispatchQueue.main.async {
                self?.refresh()
            }
        }
    ...
    private func handleIdleAlert(snapshot: StatusSnapshot) {
        ...
        if shouldTrigger {
            Telemetry.trackState("alert.idle_triggered", properties: [
                ...
                "sound_enabled": Settings.idleAlertSoundEnabled
            ])
            playIdleAlertSound()
            postIdleNotification(idlePercent: idlePercent, snapshot: snapshot)
        }
        idleAlertActive = shouldAlert
        lastIdlePercent = idlePercent
    }
    ...
    private func playIdleAlertSound() {
        guard Settings.idleAlertSoundEnabled else {
            return
        }
        if Settings.idleAlertNotificationEnabled, notificationSoundSetting == .enabled {
            return
        }
    ...
    private func postIdleNotification(
        ...
        let sendNotification = {
            ...
            if self.notificationSoundSetting == .enabled {
            ...
            Telemetry.trackState("notification.sent", properties: [
                ...
                "idle_count": snapshot.finishedCount,
                "total_count": snapshot.totalCount,
                "sound_enabled": self.notificationSoundSetting == .enabled
            ])
        }
