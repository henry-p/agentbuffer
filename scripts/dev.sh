...
hash_inputs() {
    find Sources Package.swift -type f \( -name "*.swift" -o -name "*.svg" -o -name "*.html" -o -name "*.css" -o -name "*.ts" -o -name "*.json" -o -name "Package.swift" \) -print0 \
        | xargs -0 stat -f "%m %N" \
        | LC_ALL=C sort \
        | shasum \
        | awk '{print $1}'
...
restart_app() {
    APP_PID=""
    if ! "$ROOT_DIR/scripts/build_metrics_web.sh"; then
        log "Metrics web build failed. Waiting for the next change..."
        APP_EXPECTED="false"
        return 1
    fi
